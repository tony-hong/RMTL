...
MTC_L21 <- function(X, Y, ...) UseMethod("MTC_L21")

MTC_L21.default <- function(X, Y, lam1=0.01, lam2=0, 
             opts=list(init=0, tol=10^-3, maxIter=1000)){
    task_num <- length(X)
    X <- lapply(X, function(x) as.matrix(x))
    Y <- lapply(Y, function(x) as.numeric(x))

    r <- LR_L21(X, Y, lam1, lam2, opts)
...
print.MTC_L21 <- function(x)
{
    ...
    print(x$call)
    cat("Formulation:\n")
    print('SUM_i Loss_i(W) + lam1*||W||_{2,1} + lam2*||W||{_2}{^2}')
}

predict.MTC_L21 <- function(m, newX=NULL)
{
    if(is.null(newX))
        y <- m$fitted.values
    else{
...
cv.MTC_L21 <- function(X, Y, lam2=0,
             opts=list(init=0,  tol=10^-3, maxIter=1000),
...
for (i in 1:nfolds){
    ...
    for (p_idx in 1: length(lam1)){
        r <- LR_L21(cv_Xtr, cv_Ytr, lam1=lam1[p_idx], lam2=lam2, cv_opt)
        cv_opt$init <- 1
        cv_opt$W0 <- r$W
        cv_opt$C0 <- r$C
